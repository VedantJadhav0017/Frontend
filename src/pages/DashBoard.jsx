...
function Dashboard() {
  ...
  const getWorkerById = async () => {
    try {
      ...
      const res = await axios.get(
        `/api/worker/show/getWorkerById/${currentUser.user.id}`,
        {
          headers: {
  ...
  const getAllReview = async () => {
    try {
      const token = `Bearer ${currentUser.token}`;
      const res = await axios.get(
        `/api/review/getReviewByWorkerId/${currentUser.user.id}`,
        {
          headers: {
            Authorization: String(token),
          },
  ...
  const getAllMessages = async () => {
    try {
      const token = `Bearer ${currentUser.token}`;
      const res = await axios.get(
        `https://nexum-backend-production-486e.up.railway.app/api/message/getMessageByWorkerId/${currentUser.user.id}`,
        {
          headers: {
            Authorization: String(token),
          },
      ...
      <div className="container px-5 py-24 mx-auto flex flex-wrap flex-col">
        <div className="flex mx-auto flex-wrap mb-20">
          ...
          >
            ...
            <button onClick={profileChange}>Profile</button>
          </a>
          <a
            className={`sm:px-6 py-3 w-1/2 sm:w-auto justify-center sm:justify-start border-b-2 title-font font-medium inline-flex items-center leading-none border-gray-200 hover:text-gray-900 tracking-wider  ${
              reviews ? "bg-gray-100 border-indigo-500 text-indigo-500 " : ""
            }`}
          >
            <svg
              fill="none"
            ...
            <button onClick={reviewsChange}>Reviews</button>
          </a>
          <a
            className={`sm:px-6 py-3 w-1/2 sm:w-auto justify-center sm:justify-start border-b-2 title-font font-medium inline-flex items-center leading-none border-gray-200 hover:text-gray-900 tracking-wider  ${
              messages ? "bg-gray-100 border-indigo-500 text-indigo-500 " : ""
            }`}
          >
            <svg
              fill="none"
            ...
            <button onClick={messagesChange}>Messages</button>
          </a>
          <a
            className={`sm:px-6 py-3 w-1/2 sm:w-auto justify-center sm:justify-start border-b-2 title-font font-medium inline-flex items-center leading-none border-gray-200 hover:text-gray-900 tracking-wider  ${
              personalData
          ...
          >
            ...
            <button onClick={personalDataChange}>Personal Data</button>
          </a>
        </div>
